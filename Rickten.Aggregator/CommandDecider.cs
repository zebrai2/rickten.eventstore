...
{
    private readonly Lazy<string> _aggregateName;

    /// <summary>
    /// Gets the aggregate name from the [Aggregate] attribute on TState.
    /// </summary>
    protected string AggregateName => _aggregateName.Value;

    protected CommandDecider()
    {
        _aggregateName = new Lazy<string>(() =>
        {
            var stateType = typeof(TState);
            var attr = stateType.GetCustomAttribute<AggregateAttribute>();
